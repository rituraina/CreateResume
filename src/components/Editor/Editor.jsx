...
function Editor(props) {
    ...
    const handlePointUpdate=(value, index) => {
      ...
      if(!Array.isArray(tempValues.points))tempValues.points=[0];
      tempValues.points[index] = value;
...
const projectBody =(
  <div className={styles.detail}>
    <div className={styles.row}>
    <InputControl label="Title" placeholder="Enter title eg. chat app" defaultValue={values.title}
    onChange={(event) =>
      setValues((prev)=>({...prev,title:event.target.value}))}
      />
    </div>
...
const basicInfoBody=(
  ...
  <div className={styles.row}>
    ...
    />
     <InputControl
    label="Title"
    placeholder="Enter your title name eg.Frontend developer"
    value={values.title}
    onChange={(event) =>
      setValues((prev)=>({...prev,title:event.target.value}))}
    />
...
const summaryBody=(
  <div className={styles.detail}>
    <InputControl
    label="Summary"
    placeholder="Enter your obective/summary"
    value={values.summary}
    onChange={(event) =>
      setValues((prev)=>({...prev,psummary:event.target.value}))}
    />
  </div>
);
const otherBody=(
    ...
    useEffect(()=>{
      ...
      setActiveInformation(activeInfo);
        ...
        setValues({
          ...
          title:activeInfo?.details ? activeInfo.detail?.title || ""
          :activeInfo?.detail?.title|| "",
          github:activeInfo?.details ? activeInfo.detail?.github || ""
...
setValues({
  ...
  {activeInformation?.details
    ? Object.keys(activeInformation.details).map((key, index) => (
        <div
        ...
        >
          ...
          <X onClick={(event) => handleDeleteDetail(key)} />
        </div>
      ))
    : ''}
             {
