...
<div className={styles.content}>
  {info.project?.details?.map((item)=>(
    <div className={styles.item}>
      ...
      {item.link &&(
        <a className={styles.link} href={item.link}><Paperclip/>
        {item.link}</a>
      )}
 {item.github && (
...
{item.startDate && item.endDate &&
<div className={styles.date}>
  <calender/>{""}{getFormattedDate(item.startDate)-getFormattedDate(item.endDate)}
</div>}
</div>))}
</div>
</div>,
...
<ul className={styles.points}>
{item.achievement?.points?.map((elem,index)=>(
  <li className={styles.point}key={elem + index}>{elem}</li>
))} 
...
let targetRowIndex =tempColumns[0].findIndex(item=>item===target);
 ...
 const tempSource=tempColumns[sourceColumnIndex][sourceRowIndex];
  tempColumns[sourceColumnIndex][sourceRowIndex]=tempColumns
    ...
    <div ref={containerRef} className={styles.container}>
      <div className={styles.header}>
        ...
        <div className={styles.links}>
          ...
          <a className={styles.link} type="email"><Mail/>{info.basicInfo?.detail?.email}</a>)}
          {
          ...
          <a className={styles.link}><GitHub/>{info.basicInfo?.detail?.github}</a>)}
        </div>
      </div>
